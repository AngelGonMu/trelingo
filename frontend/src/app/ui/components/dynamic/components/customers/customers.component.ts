...
export class CustomersComponent implements OnInit, OnDestroy {
  ...
  newRelated(component:string){
    let form:any;
    if(component=="todos"){
      form={customer_id: this.id, user_id: 1};
    } else {
      form={customer_id: this.id};
    }
  ...
  save() {
    let form={
      ...
      vat: (<HTMLInputElement>document.getElementById(this.component+"_vat_"+this.id)).value,
      type: (<HTMLInputElement>document.getElementById(this.component+"_type_"+this.id)).value,
  ...
  prepareLists(){
    this.lists="{";
    this._ds.list("vl", "list=customertype").subscribe(list => {
      this.lists+=`"customertype": ${JSON.stringify(list)},`;
    });
    this._ds.list("vl", "list=todotype").subscribe(list => {
      this.lists+=`"todotype": ${JSON.stringify(list)},`;
    });
    this._ds.list("vl", "list=customerstatus").subscribe(list => {
      this.lists+=`"customerstatus": ${JSON.stringify(list)}}`;
