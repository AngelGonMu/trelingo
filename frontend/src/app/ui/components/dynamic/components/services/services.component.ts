...
export class ServicesComponent implements OnInit, OnDestroy {
  ...
  search(){
    ...
    this._ds.list("products", "type=Service&search="+s+""+o).subscribe(searchedlist => {this.result=[];this.result=searchedlist;this.detail=false;});
  }
  ...
  sort(field:string){
    ...
    this._ds.list("products", "type=Service&sort="+this.sortfield+":"+this.order+""+s).subscribe(sortedlist => {this.result=[];this.result=sortedlist;this.detail=false;});
  }

  list() {
    this._ds.list("products", "type=Service").subscribe(list => {this.result=list;this.detail=false;});
  }

  get() {
    this._ds.get("products", this.id).subscribe(item => {this.result=item;this.detail=true;});
  }

  new(){
    this._ds.store("products", {type: "Service"}).subscribe(item => this.router.navigate([`/ui/${this.module.parent}/${this.component}/${item.result.id}`]));
  }

  delete(id:number){
    this._ds.delete("products", id).subscribe(data => {this.router.navigate([`/ui/${this.module.parent}/${this.component}`]);this.list();});
  }
  ...
  save() {
    ...
    this._ds.update("products", this.id, form).subscribe(item => this.result=item);
  }
